...
public class SList extends List {
  ...
  /**
   * Sees if there is item inside the current list
   * @param item
   * @return return the node if there is, else return null
   */
  
  public SListNode find(Position item) {
	  SListNode currNode = head.next;
	  
	  try { 
		  while(currNode.isValidNode()) {
			  Position currPos = (Position) currNode.item();
			  if(currPos.getX() == item.getX() && currPos.getY() == item.getY()) {
				  return currNode;
			  } else {
